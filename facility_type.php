...
$id= empty($_REQUEST['id']) ? null : $_REQUEST['id'];
 ...
 {
	saveSimpleSetup('facility_types',$id);
 }

 if(isset($_REQUEST['id']))
 {
	$item = getSimpleSetup('facility_types',$_REQUEST['id']);
	$editname = $item->name;
 ...
 $lst = getSimpleSetup('facility_types');

 if(!empty($_SESSION['countryid']))
 {
	$lst = getData('facility_types',array('countryid=?',$_SESSION['countryid']));
	$ctlst = getData('country',array('id=?',$_SESSION['countryid']));
 }

 if(!empty($_REQUEST['countryid']) && $_REQUEST['countryid'] > 0)
 {
	$lst = getData('facility_types',array('countryid=?',$_REQUEST['countryid']));
 }
...
?>
		...
		<div id="content" class="content">
			...
			<!-- end breadcrumb -->
			<!-- begin page-header -->
			<h1 class="page-header"><small><?php echo lang('dataEntry.FACILITY_TYPE')?></small></h1>
			<!-- end page-header -->
        ...
        <div class="row">
            <div class="col-xl-6 ui-sortable">
                <div class="panel panel-inverse">
				    <div class="panel-heading">
					    <h4 class="panel-title"><?php echo lang('dataEntry.FAC_TYPE_LIST')?></h4>
                        ...
                        <table id="data-table-fixed-columns" class="table table-striped table-bordered table-td-valign-middle" style="width:100%;">
					        ...
					        <tbody>
								<?php
									 ...
									 {?>
									  	<tr>
										  	<td> <?php echo getSimpleSetup('country',$lst->countryid)->name ; ?></td>
											<td> <?php echo $lst->name; ?></td>
											<td> <?php echo $lst->is_active == 1? 'Yes':'No'; ?></td>
											<td><a href="facility_type.php?id=<?php echo $lst->id; ?>"> <?php echo lang('dataEntry.EDIT')?></a></td>
										</tr>
									 <?php
										 $lst->next();
									 }
            ...
            <div class="col-xl-6 ui-sortable">
                <div class="panel panel-inverse">
				    <div class="panel-heading">
					    <h4 class="panel-title"><?php echo lang('dataEntry.ADD_FAC_TYPE')?></h4>
					    <div class="panel-heading-btn">
						    <a href="javascript:;" class="btn btn-xs btn-icon btn-circle btn-default" data-click="panel-expand"><i class="fa fa-expand"></i></a>
						    <a href="javascript:;" class="btn btn-xs btn-icon btn-circle btn-success" data-click="panel-reload"><i class="fa fa-redo"></i></a>
						    <a href="javascript:;" class="btn btn-xs btn-icon btn-circle btn-warning" data-click="panel-collapse"><i class="fa fa-minus"></i></a>
						        ...
						        <label class="col-form-label col-md-4"><?php echo lang('general.COUNTRY')?> :</label>
									...
									<select class="form-control" name="countryid" onchange="self.location='<?php echo $_SERVER['PHP_SELF'];?>?countryid='+this.value">
											...
											<?php
												while(!$ctlst ->dry())
												{?>
													<option <?php if(!empty($_REQUEST['countryid']) && $_REQUEST['countryid'] == $ctlst->id) echo 'selected'; else if($$countryid && $$countryid == $ctlst->id) echo 'selected'; ?> value="<?php echo $ctlst->id; ?>"><?php echo $ctlst->name; ?></option>
												<?php
												 $ctlst->next();
												}
											?>
