...
$id= empty($_REQUEST['id']) ? null : $_REQUEST['id'];
 ...
 {
	saveSimpleSetup('regions',$id);
 }

 if(isset($_REQUEST['id']))
 {
	$item = getSimpleSetup('regions',$_REQUEST['id']);
	$editname = $item->name;
 ...
 $lst = getSimpleSetup('regions');
 if(!empty($_SESSION['countryid']))
 {
	$lst = getData('regions',array('countryid=?',$_SESSION['countryid']));
 }
...
?>
		...
		<div id="content" class="content">
			...
			<!-- end breadcrumb -->
			<!-- begin page-header -->
			<h1 class="page-header"><small><?php echo lang('dataEntry.REGIONS')?></small></h1>
			<!-- end page-header -->
        ...
        <div class="row">
            <div class="col-xl-6 ui-sortable">
                <div class="panel panel-inverse">
				    <div class="panel-heading">
					    <h4 class="panel-title"><?php echo lang('dataEntry.REG_LIST')?></h4>
								...
								<?php
									 ...
									 {?>
									  	<tr>
											...
											<td><a href="regions.php?id=<?php echo $lst->id; ?>"> <?php echo lang('dataEntry.EDIT')?></a></td>
										</tr>
									 <?php
										 $lst->next();
									 }
            ...
            <div class="col-xl-6 ui-sortable">
                <div class="panel panel-inverse">
				    <div class="panel-heading">
					    <h4 class="panel-title"><?php echo lang('dataEntry.ADD_REG')?></h4>
					    <div class="panel-heading-btn">
						    <a href="javascript:;" class="btn btn-xs btn-icon btn-circle btn-default" data-click="panel-expand"><i class="fa fa-expand"></i></a>
						    <a href="javascript:;" class="btn btn-xs btn-icon btn-circle btn-success" data-click="panel-reload"><i class="fa fa-redo"></i></a>
						    <a href="javascript:;" class="btn btn-xs btn-icon btn-circle btn-warning" data-click="panel-collapse"><i class="fa fa-minus"></i></a>
						        ...
						        <label class="col-form-label col-md-4"><?php echo lang('general.COUNTRY')?> :</label>
									...
									<select class="form-control" name="countryid">
											<?php
												while(!$ctlst ->dry())
												{?>
													<option <?php if(!empty($country) && $country == $ctlst->id) echo 'selected'  ?> value="<?php echo $ctlst->id; ?>"><?php echo $ctlst->name; ?></option>
												<?php
												 $ctlst->next();
												}
											?>
