...
$id= empty($_REQUEST['id']) ? null : $_REQUEST['id'];
 ...
 $lst = getData('eqa_data_entry',array('type=?','pt_national'));
 ...
 $plst = getData('providers',array('countryid=? and type=?', $clst->id,'national'));
 ...
 if(!empty($_SESSION['countryid']))
 {
	...
	$plst = getData('providers',array('countryid=? and type=?',$_SESSION['countryid'],'national'));
 }
 ...
 if(!empty($_REQUEST['countryid']) && $_REQUEST['countryid'] > 0)
 {
	...
	$plst = getData('providers',array('countryid=? and type=?',$_REQUEST['countryid'],'national'));
 }
...
?>
		...
		<div id="content" class="content">
			...
			<!-- end breadcrumb -->
			<!-- begin page-header -->
			<h1 class="page-header"><small><?php echo lang('dataEntry.NAT_EQA_DATA')?></small></h1>
			<!-- end page-header -->
        ...
        <div class="col-xl-8 ui-sortable">
                <div class="panel panel-inverse">
				    <div class="panel-heading">
					    <h4 class="panel-title"><?php echo lang('dataEntry.NAT_EQA_LIST')?></h4>
							...
							<?php
									 ...
									 {?>
									  	<tr>
											...
											<td><a href="natpteqa_data.php?id=<?php echo $lst->id.'&countryid='.$lst->country_id;; ?>"> <?php echo lang('dataEntry.EDIT')?></a></td>
										</tr>
									 <?php
										 $lst->next();
									 }
						        ...
						        <label class="col-form-label col-md-4"><?php echo lang('general.REAS_REJ')?> :</label>
									...
									<option value=""></option>
											<?php
												while(!$rlst->dry())
												{?>
													<option <?php if($item->reason_id==$rlst->id) echo 'selected'; ?> value="<?php echo $rlst->id; ?>"><?php echo $rlst->name; ?></option>
												<?php
												 $rlst->next();
												}
											?>
